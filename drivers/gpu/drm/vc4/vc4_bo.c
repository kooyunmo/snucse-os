...
struct vc4_gem_info_data {
	struct drm_file *filp;
	struct seq_file *m;
};

static int vc4_gem_one_info(int id, void *ptr, void *data)
{
...
int vc4_debugfs_gem_info(struct seq_file *m, void *data)
{
	...
	struct vc4_gem_info_data gem_info_data;
	struct drm_file *filp;
	...
	list_for_each_entry(filp, &drm_dev->filelist, lhead) {
		...
		idr_for_each(&filp->object_idr, vc4_gem_one_info,
				&gem_info_data);
		    ...
		    struct drm_mode_create_dumb *args)
{
	...
	if (!ret)
		vc4_gem_register_pid(file_priv);
