        ...
        struct task_struct *p, int new_cpu)
{
    ...
    enqueue_task(rq, p, new_cpu);
    p->on_rq = TASK_ON_RQ_QUEUED;
    check_preempt_curr(rq, p, new_cpu);
...
void scheduler_tick(void)
{
    ...
    if(curr->policy == SCHED_WRR) {
        ...
        if(time_after_eq(jiffies, prev_jiffies + (2 * HZ))){
            printk(KERN_ALERT "*************** before load balance *************\n");
            print_curr_cpu_weights();
            preempt_disable();
            wrr_load_balance();
            preempt_enable();
            printk(KERN_ALERT "*************** after load balance **************\n");
            print_curr_cpu_weights();
        ...
        const struct sched_attr *attr, bool keep_boost)
{
    ...
    if (dl_prio(p->prio))
        ...
        /*if (cpumask_equal(&p->cpus_allowed, cpu_all_mask))
            do_set_cpus_allowed(p, &hmp_slow_mask);*/
    }
    else
        p->sched_class = &fair_sched_class;
}
...
long sched_setweight(pid_t pid, int weight) {
    printk(KERN_ALERT "sched_setweight: weight: %d\n", weight);
	...
	if (!p || !rq || p->policy!=7)
        return -EINVAL;
	if(!uid_eq(current_uid(), task_uid(p)) && !uid_eq(current_uid(), KUIDT_INIT(0))){
		return -EPERM;
	}
	if(p->wrr.weight < weight && !uid_eq(current_uid(), KUIDT_INIT(0))){
